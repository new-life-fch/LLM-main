"""集成CausalEditor的Llama-2模型
基于原有的modeling_llama_causal.py，针对Llama-2-7b-chat模型进行优化
主要改进：
1. 针对Llama-2的架构特点进行优化
2. 调整默认参数以适应7B模型规模
3. 增强对Llama-2特有特性的支持
4. 优化内存使用和性能
"""
...
class CausalLlama2Attention(LlamaAttention):
    """集成CausalEditor的Llama-2 Attention层
    针对Llama-2的架构特点进行优化
    """
...
class CausalLlama2MLP(LlamaMLP):
    """集成CausalEditor的Llama-2 MLP层
    针对Llama-2的架构特点进行优化
    """
...
class CausalLlama2DecoderLayer(LlamaDecoderLayer):
    """集成CausalEditor的Llama-2 Decoder层
    针对Llama-2的架构特点进行优化
    """
    ...
    def __init__(self, config, layer_idx: int):
        ...
        # 替换attention和mlp为我们的Llama-2版本
        self.self_attn = CausalLlama2Attention(config, layer_idx)
        self.mlp = CausalLlama2MLP(config, layer_idx)
...
class CausalLlama2Model(LlamaModel):
    """集成CausalEditor的Llama-2模型
    针对Llama-2的架构特点进行优化
    """

    def __init__(self, config):
        # 保持原始的预训练配置以确保与Llama-2权重的兼容性
        super().__init__(config)

        # 替换layers为我们的Llama-2版本
        self.layers = torch.nn.ModuleList(
            [
                CausalLlama2DecoderLayer(config, layer_idx)
...
class CausalLlama2ForCausalLM(LlamaForCausalLM):
    """集成CausalEditor的Llama-2因果语言模型
    针对Llama-2-7b-chat模型进行优化
    """
    ...
    def __init__(self, config):
        ...
        # 替换模型为我们的Llama-2版本
        self.model = CausalLlama2Model(config)
    ...
    def from_pretrained_with_dynamic_causal_editor(
        ...
        **kwargs,
        ...
        # 加载基础Llama-2模型
        model = cls.from_pretrained(model_name_or_path, **recommended_kwargs)
        ...
        # 针对Llama-2-7b模型初始化动态CausalEditor
        ...
        logging.info(f"CausalLlama2模型已加载并配置动态CausalEditor")
                ...
                try:
                    ...
                    if input_ids.numel() > 0:
                        ...
                        if user_input_text is not None and hasattr(self.model, 'set_generation_context'):
                            # 读取上下文窗口：默认64，可由 rag_config['conflict_detector']['token_window'] 覆盖
                            context_window = 64
                            try:
                                if hasattr(self, 'causal_editor') and self.causal_editor:
                                    ...
                                    if cfg_window is not None:
                                        try:
                                            ...
                                            logging.debug(f"Invalid token_window in rag_config: {cfg_window}, using default 64")
                            except Exception as cfg_e:
                                logging.debug(f"Falling back to default context_window=64 due to config error: {cfg_e}")
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            info = {
                "model_type": "CausalLlama2ForCausalLM",
...
# 兼容性别名
CausalLlama2 = CausalLlama2ForCausalLM

# 为了向后兼容，也提供原始名称的别名
CausalLlamaForCausalLM_Llama2 = CausalLlama2ForCausalLM